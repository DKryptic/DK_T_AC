...
namespace DK_T_AC
{
    ...
    public class PMCRW
    {
        ...
        public byte[] readBytes(int MemoryAddress, uint iNumofBytesToRead)
        {
            byte[] buffer;
            int read = ReadMem(MemoryAddress, iNumofBytesToRead, out buffer);
            if (read == 0)
                return buffer;
            else {
                byte[] bufferfixed = new byte[4];
                bufferfixed = buffer.Reverse().ToArray();
                return bufferfixed;
            }
        }

        public string readString(int MemoryAddress, uint iNumofBytesToRead)
        {   // We did the split at the end of the convertion to string to remove null terminators ('\0').
            // Should cover every scenario, and seems to work good.
            byte[] buffer;
            int read = ReadMem(MemoryAddress, iNumofBytesToRead, out buffer);
            if (read == 0)
                return Encoding.UTF8.GetString(buffer, 0, buffer.Length).Split(new[] { '\0' }, 2)[0];
            else
                return Encoding.UTF8.GetString(buffer, 0, buffer.Length).Split(new[] { '\0' }, 2)[0];
        }
        ...
        public void WriteBytes(int MemoryAddress, byte[] b)
        {
            WriteMem(MemoryAddress, b);
        }
        ...
        public string GetModuleBaseAddress(int iProcId, string processName, string moduleName)
        {
            ...
            try
            {
                process = Process.GetProcessById(iProcId);
            }
            ...
            string outputHex = process.MainModule.ModuleName + "-" + process.MainModule.BaseAddress.ToString("x8");
            return outputHex;
        }
    }
